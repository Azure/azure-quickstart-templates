...
oc new-project clustered-jboss-eap --display-name="A Clustered JBoss EAP application" --description="This is a clustered JBoss EAP application with session replication on OCP"
oc policy add-role-to-user view system:serviceaccount:$(oc project -q):default -n $(oc project -q)
oc policy add-role-to-user view system:serviceaccount:$(oc project -q):eap-service-account -n $(oc project -q)
oc policy add-role-to-user view system:serviceaccount:$(oc project -q):eap-service-account -n $(oc project -q)
oc new-app -f https://raw.githubusercontent.com/danieloh30/eap-session-replication/master/ocp-template/eap71-basic-s2i.json
