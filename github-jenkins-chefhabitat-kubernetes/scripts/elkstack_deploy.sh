...
wget https://raw.githubusercontent.com/sysgain/MSOSS/kubstage/scripts/default -O /etc/nginx/sites-available/default >> $LOG
sudo nginx -t >> $LOG
...
wget https://raw.githubusercontent.com/sysgain/MSOSS/kubstage/scripts/02-beats-input.conf -O /etc/logstash/conf.d/02-beats-input.conf >> $LOG
sudo ufw allow 5044 >> $LOG
wget https://raw.githubusercontent.com/sysgain/MSOSS/kubstage/scripts/10-syslog-filter.conf -O /etc/logstash/conf.d/10-syslog-filter.conf >> $LOG
wget https://raw.githubusercontent.com/sysgain/MSOSS/kubstage/scripts/30-elasticsearch-output.conf -O /etc/logstash/conf.d/30-elasticsearch-output.conf >> $LOG
