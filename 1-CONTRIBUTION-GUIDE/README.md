...
1. The **azuredeploy.json** file must still be included, and must be generated by the bicep compiler (use this command: `build main.bicep --outfile azuredeploy.json`)
1. The **README.md** file must include a link to the bicep badge (see [sample-README.md](sample-README.md))
1. The parameter file must still be named **azuredeploy.parameters.json**

An easy way to convert an existing sample to support Bicep is to use the decompiler:

```sh
bicep decompile azuredeploy.json --outfile main.bicep
```

The decompiler is not guaranteed to produce correct bicep from JSON, so you will need to inspect and modify the **main.bicep** file afterwards (and use `bicep build` afterwards to generate the matching **azuredeploy.json** file). Some general guidelines:

1. Rename (F2) parameters and variables to be camel-cased.
1. Rename resource symbolic names to a logical, short name (and do not use `Name` in the symbolic name)
