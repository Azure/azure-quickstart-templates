...
sudo docker run -v $(pwd):/scripts --network=host -e STORAGE_ACCOUNT=${storage_account} -e CONTAINER_NAME=${container_name} -e RESOURCE_GROUP=${resource_group} -e PORT=${PORT} ${docker_image} /bin/bash /scripts/${script_file}