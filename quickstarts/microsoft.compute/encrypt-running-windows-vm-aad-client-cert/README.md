---
description: This template enables encryption on a running windows vm using Microsoft Entra client cert thumbprint. The certificate should have been deployed to the VM earlier
page_type: sample
products:
- azure
...
- json
---
# Enable encryption on a running Windows VM and Microsoft Entra ID
...
This template enables encryption on a running windows vm using Microsoft Entra client cert thumbprint. The certificate should have been deployed to the VM before running this template. This template assumes that the VM is located in the same region as the resource group. If not, please edit the template to pass appropriate location for the VM sub-resources.
