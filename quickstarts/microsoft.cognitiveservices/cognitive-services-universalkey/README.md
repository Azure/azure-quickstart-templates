...
- json
---
# Deploy an Azure AI services multi-service resource (classic)
...
> [!IMPORTANT]
> **Microsoft Foundry is the successor to Azure AI services** including this multi-service (`CognitiveServices` kind, also known as the "universal key") resource, and composes capabilities from Azure OpenAI, Azure Speech, and Azure Language in a single account. For new workloads, provisioning a Microsoft Foundry account is strongly recommended over standalone Azure AI service resource types so you benefit from the latest model releases, capabilities, and developer scenarios that span across all services. See the [Deploy a Microsoft Foundry account (basic)](../foundry-getting-started/README.md) sample to get started.
