...
var imageGalleriesRaw = !empty(imageGalleries) ? imageGalleries : [
  {
...
var imageGalleriesFinal = [for item in imageGalleriesRaw: {
  galleryName: item.galleryName
  gallerySubscriptionId: contains(item, 'gallerySubscriptionId') ? item.gallerySubscriptionId : gallerySubscriptionId
  galleryResourceGroup: contains(item, 'galleryResourceGroup') ? item.galleryResourceGroup : galleryResourceGroup
}]
...
var defaultImagePublishingProfile = union({
    targetRegions: empty(targetRegions) ? [ { name: location, replicas: 1 } ] : map(targetRegions, targetRegion => { name: targetRegion, replicas: 1 })
  },
