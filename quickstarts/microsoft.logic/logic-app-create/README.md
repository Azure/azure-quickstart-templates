# Create a Consumption logic app workflow with a quickstart ARM template
...
Azure Logic Apps is a cloud service that automates the execution of your business processes. You can create a workflow by using a visual designer to arrange prebuilt components into the sequence that you need. When you save your workflow, the designer sends the definition for your workflow to the Azure Logic Apps execution engine. When workflow's trigger conditions are met, the engine launches your workflow and manages the compute resources required by the workflow needs to run. If you're new to Azure Logic Apps, see [What is Azure Logic Apps?](https://docs.microsoft.com/azure/logic-apps/logic-apps-overview).

This quickstart template creates a basic [Consumption logic app workflow](https://docs.microsoft.com/azure/logic-apps/logic-apps-overview#resource-type-differences) that runs in multi-tenant Azure Logic Apps and connects to the status update feed for Azure services at https://status.azure.com/en-us/status/feed, which is displayed at https://status.azure.com/en-us/status.
