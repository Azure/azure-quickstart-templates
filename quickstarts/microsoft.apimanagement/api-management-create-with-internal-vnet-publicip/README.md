...
This template shows an example of how to deploy an Azure API Management service within your own virtual network's subnet in [internal mode](https://learn.microsoft.com/azure/api-management/api-management-using-with-internal-vnet).
...
- The template creates a Premium tier API Management instance that is deployed to two [availability zones](https://learn.microsoft.com/azure/api-management/zone-redundancy). You may choose to deploy the API Management instance in the Developer tier; however, availability zones are not supported in that tier.
