...
- json
---
# creates an Azure Stack HCI 24H2 cluster with SAN Storage

This template allows you to create an Azure Stack HCI cluster using version 24H2 with SAN Storage. First you deploy the template in validate mode which does confirm the parameters at the device. Once passed you re-deploy the template with mode set to deploy.

For a RAC SAN cluster, set `witnessType` to `ClusterWitnessDisk` and provide the witness volume LUN ID in `witnessLunId`. The witness LUN must be at least 1 GB.
