...
if($p.Count -gt 3) { # for master only
...
echo "start worker (total cores $total)" | trace
for($i=0;$i -lt $total;$i++) {
  $workername = "WORKER_" + $env:COMPUTERNAME + "_" + $i + "_" + $total
  echo "add worker $workername" | trace
  .\startworker.bat -jobmanagerhost $masterfqdn -jobmanager mymjs -name $workername -v 2>&1 | trace
}
