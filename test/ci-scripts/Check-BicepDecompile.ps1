...
foreach ($f in $bicepFiles){
    ...
    $bicepText | Select-String -Pattern "resource \w{1,}_resource | \w{1,}_var | \w{1,}_param" -AllMatches |
        foreach-object {$_.Matches} | foreach-object {
            Write-Warning "$($f.Name) may contain raw output from decompile: $($_.Value)"
            # write the environment var
            Write-Host "##vso[task.setvariable variable=bicep.decompile]$true"
        }
}
