...
then
 echo "#########################################################################################################################"
 echo "## MYSQL ROOT PASSWORD OR DEBIAN SYSMAINT PASSWORD IS EMPTY. UNLESS THIS IS INTENTIONAL, HIT CTRL + C WITHIN 30 SECONDS!"
 echo "#########################################################################################################################"
 sleep 30
fi
...
then
 ...
 ssh $DBNODEUSER@$FIRST "sudo cp /etc/mysql/debian.cnf ~/debian.cnf"
