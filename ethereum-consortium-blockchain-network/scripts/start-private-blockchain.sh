...
if [ $NODE_TYPE -eq 0 ]; then
  ...
  nohup nodejs app.js $ADMIN_SITE_PORT $GETH_HOME/geth.ipc $ETHERBASE_ADDRESS $PASSWD $MN_NODE_PREFIX $NUM_MN_NODES $TX_NODE_PREFIX $NUM_TX_NODES $NUM_BOOT_NODES >> $ETHERADMIN_LOG_FILE_PATH 2>&1 &
  if [ $? -ne 0 ]; then echo "Previous command failed. Exiting"; exit $?; fi
