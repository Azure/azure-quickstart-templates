...
echo "Red Hat JBoss EAP 7 Clustered Intallation Start: " | /bin/date +%H:%M:%S  >> /home/$1/install.log
...
/bin/cp -rf /home/$1/eap-session-replication/eap-configuration/standalone-ha.xml $JBOSS_HOME/standalone/configuration/
/bin/cp -rf /home/$1/eap-session-replication/target/eap-session-replication.war $JBOSS_HOME/standalone/deployments/eap-session-replication.war
...
$JBOSS_HOME/bin/standalone.sh -c $SVR_CONFIG -b $IP_ADDR -bmanagement $IP_ADDR -bprivate $IP_ADDR -Djboss.jgroups.azure_ping.storage_account_name=$STORAGE_ACCOUNT_NAME -Djboss.jgroups.azure_ping.storage_access_key=$STORAGE_ACCESS_KEY -Djboss.jgroups.azure_ping.container=$CONTAINER_NAME > /dev/null 2>&1 &
...
echo "Red Hat JBoss EAP 7 Clustered Intallation End: " | /bin/date +%H:%M:%S  >> /home/$1/install.log